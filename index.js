...
global.db.data.chats[m.chat].mute = true
        reply(`Okey, Bot Nya Udah Di Ban Di Grup Ini`)
break
...
if (!global.db.data.chats[m.chat].mute) return reply(`Bot belum di ban dalam group ini`)
        ...
        reply(`Okey Bot Berhasil Di Unban Chat`)
break
...
if (!m.isGroup) return reply(lang.groupOnly())
        ...
        global.db.data.chats[m.chat].mute = true
        reply(`Okey, Bot Nya Udah Di Mute Di Grup Ini`)
        } else if (args[0] === "off") {
        ...
        reply(`Oke Darling Sekarang Sudah Nonaktif !`)
            ...
            case 'delete': case 'del': {
                ...
                if (!isBaileys) throw 'Pesannya Bukan Bot Yg Ngirim🗿 !'
	    ...
	    case 'toimage': case 'toimg': {
            ...
            case 'public': {
                if (!m.key.fromMe && !isCreator) return reply(lang.ownerOnly())
                alpha.public = true
                reply('Sukses Ganti Ke Mode Public Darling:v')
            }
            break
            case 'self': {
                if (!m.key.fromMe && !isCreator) return reply(lang.ownerOnly())
                alpha.public = false
                reply('Sukses Ganti Ke Mode Self\n\nUntuk mengubah ke mode public silahkan gunakan nomor bot Ya Darling')
            }
            break
...
break
            ...
            case 'out':
                if (!m.isGroup) return reply(lang.groupOnly())
                if (!m.key.fromMe && !isCreator) return reply(lang.ownerOnly())
               reply('Sayonara M~ 👋').then(async res => await alpha.groupLeave(from))
            break
...
break
            ...
            break
				case 'menu':{
					...
					break    
	case 'allmenu':{
			await alpha.send5ButLoc(from, `Hai Darling ${pushname} 👋, Watashi *${botname}* ` + '\n\n' + lang.listMenu(time, salam, pushname, prefix) , `© ${ownername}`,pp_bot, [{"urlButton": {"displayText": "YouTube Saya","url": `${youtube}`}},{"urlButton": {"displayText": "Klik dong","url": `${myweb}`}},{"quickReplyButton": {"displayText": "Donasi Dong","id": 'donate'}},{"quickReplyButton": {"displayText": "My Darling","id": 'owner'}},{"quickReplyButton": {"displayText": "Rulesnya","id": 'rules'}}] )
		}
	break   
